...
function getCoord(e) {
	var 	carPos = usuario.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(usuario),
	...
	div.textContent = usuario.value.substr(0, carPos);
	span.textContent = usuario.value.substr(carPos) || '.';
	div.appendChild(span);
	
	emailCoords = getPosition(usuario);							//console.log("emailCoords.x: " + emailCoords.x + ", emailCoords.y: " + emailCoords.y);
...
function onEmailInput(e) {
	...
	// very crude usuario validation for now to trigger effects
	if(curEmailIndex > 0) {
...
usuario.addEventListener('focus', onEmailFocus);
usuario.addEventListener('blur', onEmailBlur);
usuario.addEventListener('input', onEmailInput);
